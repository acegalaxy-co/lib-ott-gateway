 ...
 */
function createTelegramRateLimiter({ globalIntervalMs = 35, channelIntervalMs = 1000, channelTtlMs = 600000, } = {}) {
    let globalNextAt = 0;
    const channelNextAt = new Map();
    function cleanup(now) {
        for (const [k, t] of channelNextAt) {
            if (t + channelTtlMs < now)
                channelNextAt.delete(k);
        }
    }
    async function acquireSlot(chatId) {
        const key = String(chatId || "*");
        // Per-channel slot
        let now = Date.now();
        if (channelNextAt.size > 64)
            cleanup(now);
 ...
 */
async function sendMessageWithRetry({ apiBase, token, chatId, text, extra = {}, limiter, maxRetry = 3, timeoutMs = 15000, }) {
    if (!token)
        throw new Error("telegram-rate: token missing");
...
module.exports = { createTelegramRateLimiter, sendMessageWithRetry };
